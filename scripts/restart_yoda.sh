...
echo "y" | odind tx bank send $1 $(yoda keys show reporter) 1000000loki --broadcast-mode block --keyring-backend test --chain-id odin --node $2

echo "y" | odind tx oracle add-reporters $(yoda keys show reporter) --from $1 --broadcast-mode block --keyring-backend test --chain-id odin --node $2
