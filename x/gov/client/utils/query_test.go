...
import (
	...
	govutils "github.com/GeoDB-Limited/odin-core/x/gov/client/utils"
...
func (mock TxSearchMock) TxSearch(ctx context.Context, query string, prove bool, page, perPage *int, orderBy string) (*ctypes.ResultTxSearch, error) {
	...
	if start < 0 || end < 0 {
		// nil result with nil error crashes govutils.QueryTxsByEvents
...
func TestGetPaginatedVotes(t *testing.T) {
	...
	for _, tc := range []testCase{
		...
		t.Run(tc.description, func(t *testing.T) {
			...
			votesData, err := govutils.QueryVotesByTxQuery(clientCtx, params)
			require.NoError(t, err)
			votes := []types.Vote{}
