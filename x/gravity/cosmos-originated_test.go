...
func lockCoinsInModule(tv *testingVars) {
	var (
		userCosmosAddr, err           = sdk.AccAddressFromBech32("odin10e7tx3lsjl5yaw0sqxlaeg98ghm2k90e6g47x8")
		denom                         = "ugraviton"
...
func acceptDepositEvent(tv *testingVars) {
	var (
		myCosmosAddr, err = sdk.AccAddressFromBech32("odin10e7tx3lsjl5yaw0sqxlaeg98ghm2k90e6g47x8")
		myNonce           = uint64(3)
		anyETHAddr        = "0xf9613b532673Cc223aBa451dFA8539B87e1F666D"
	)
	require.NoError(tv.t, err)

	startBalance := tv.input.BankKeeper.GetAllBalances(tv.ctx, myCosmosAddr)
	...
	assert.Equal(tv.t,
		...
		tv.input.BankKeeper.GetAllBalances(tv.ctx, myCosmosAddr).Sub(startBalance))
