...
func exportAppStateAndTMValidators(
	...
	if height != -1 {
		bandApp := app.NewBandApp(logger, db, traceStore, false, uint(1), map[int64]bool{}, "", viper.GetBool(flagDisableFeelessReports), viper.GetString(flagWithSyncLog))
	...
	bandApp := app.NewBandApp(logger, db, traceStore, true, uint(1), map[int64]bool{}, "", viper.GetBool(flagDisableFeelessReports), viper.GetString(flagWithSyncLog))
	return bandApp.ExportAppStateAndValidators(forZeroHeight, jailWhiteList)
}
