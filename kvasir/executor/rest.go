...
func (e *RestExec) Exec(arg string, env interface{}) (ExecResult, error) {
	...
	// TODO: find better solution and refactor
	r := externalExecutionResponse{}

	now := time.Now()
	name := fmt.Sprintf("%d-%d.png", now.Second(), now.Nanosecond())

	err = resp.DownloadToFile(name)
	if err != nil {
		return ExecResult{}, err
	}

	if r.Returncode == 0 {
		return ExecResult{Output: []byte(name), Code: 0, Version: r.Version}, nil
	} else {
