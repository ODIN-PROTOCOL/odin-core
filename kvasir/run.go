...
func runImpl(c *Context, l *Logger) error {
	...
	for _, contract := range c.contracts {
		requests, err := queryPendingRequests(c, l, contract)
		if err != nil {
			l.Error(":exploding_head: Failed to get pending requests from contract %s: %s", c, contract, err.Error())
		}

		pendingRequests[contract] = requests
		pendingRequestsCount += len(requests)
	}

	l.Info(":mag: Found %d pending requests", pendingRequestsCount)
	for contract, requests := range pendingRequests {
...
func runCmd(c *Context) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			c.validatorAccAddr, err = sdk.AccAddressFromBech32(cfg.ValidatorAccAddr)
			if err != nil {
				return err
			}
			err = sdk.VerifyAddressFormat(c.validatorAccAddr)
			if err != nil {
				return err
			}
			...
			c.pendingRequests = make(map[RequestKey]bool)
...
func queryPendingRequests(c *Context, l *Logger, contractAddr string) ([]Request, error) {
	query := "{\"list_pending_requests\":{}}"

	client := wasmtypes.NewQueryClient(c.grpc)
	res, err := client.SmartContractState(context.Background(), &wasmtypes.QuerySmartContractStateRequest{
...
func queryPendingRequest(c *Context, l *Logger, requestID uint64, contractAddr string) (Request, error) {
	query := fmt.Sprintf("{\"get_request\":{\"request_id\": %d}}", requestID)
