...
type TestChain struct {
	...
	QueryServer   ibctypes.QueryServer
...
func NewTestChainWithValSet(t *testing.T, coord *Coordinator, chainID string, valSet *cmttypes.ValidatorSet, signers map[string]cmttypes.PrivValidator) *TestChain {
	t.Helper()
	genAccs := make([]authtypes.GenesisAccount, 0)
	genBals := make([]banktypes.Balance, 0)
	senderAccs := make([]SenderAccount, 0)

	// generate genesis accounts
	for i, _ := range valSet.Validators {
		senderPrivKey := secp256k1.GenPrivKey()

		acc := authtypes.NewBaseAccount(senderPrivKey.PubKey().Address().Bytes(), senderPrivKey.PubKey(), uint64(i*2), 0)
		amount, ok := math.NewIntFromString("10000000000000000000")
		require.True(t, ok)

		valAcc := authtypes.NewBaseAccount(testapp.Validators[i].Address, testapp.Validators[i].PubKey, uint64(i*2+1), 0)

		// add sender account
		balance := banktypes.Balance{
			Address: acc.GetAddress().String(),
			Coins:   sdk.NewCoins(sdk.NewCoin("loki", amount), sdk.NewCoin("minigeo", amount)),
		}

		valBalance := banktypes.Balance{
			Address: testapp.Validators[i].Address.String(),
			Coins:   sdk.NewCoins(sdk.NewCoin("loki", amount), sdk.NewCoin("minigeo", amount)),
		}

		genAccs = append(genAccs, acc, valAcc)
		genBals = append(genBals, balance, valBalance)
	...
	app := testapp.SetupWithGenesisValSet(t, valSet, genAccs, chainID, sdk.DefaultPowerReduction, genBals...)
	...
	for _, v := range vals {
		...
		require.NoError(t, err)
		err = app.OracleKeeper.Activate(ctx, valAddr)
		require.NoError(t, err)
	}
	...
	exp := chain.LastHeader.Header.Time.Add(1 * time.Hour)

	for i, _ := range vals {
		msg, err := authz.NewMsgGrant(
			testapp.Validators[i].Address,
			senderAccs[0].SenderAccount.GetAddress(),
			authz.NewGenericAuthorization(sdk.MsgTypeURL(&types.MsgReportData{})),
			&exp,
		)
		require.NoError(t, err)
		err = chain.sendMsgs(&testapp.Validators[i].PrivKey, msg)
		require.NoError(t, err)
	}
	// odin1ed4302ch7rx7wsggs4e9q5ez8pcas2r5j2umdn
	return chain
}
...
func (chain *TestChain) sendMsgs(sender *cryptotypes.PrivKey, msgs ...sdk.Msg) error {
	_, err := chain.SendMsgs(sender, msgs...)
	return err
}
...
// occurred.
func (chain *TestChain) SendMsgs(sender *cryptotypes.PrivKey, msgs ...sdk.Msg) (*abci.ExecTxResult, error) {
	if chain.SendMsgsOverride != nil {
		return chain.SendMsgsOverride(msgs...)
	}

	accountNumber := []uint64{chain.SenderAccount.GetAccountNumber()}
	accountSeq := []uint64{chain.SenderAccount.GetSequence()}

	if sender == nil {
		sender = &chain.SenderPrivKey
		...
		senderAcc := chain.App.AccountKeeper.GetAccount(chain.GetContext(), sdk.AccAddress((*sender).PubKey().Address()))
		accountNumber = []uint64{senderAcc.GetAccountNumber()}
		accountSeq = []uint64{senderAcc.GetSequence()}
	}

	// ensure the chain has the latest time
	chain.Coordinator.UpdateTimeForChain(chain)
	// odin1k2yh69mjeperzjutmqjmatpzxk4cqvkqgcqsp0
...
func (chain *TestChain) SendReport(
	...
	report := types.NewMsgReportData(rid, rawReps, sender)
	msg := authz.NewMsgExec(chain.SenderAccount.GetAddress(), []sdk.Msg{report})

	return chain.SendMsgs(nil, &msg)
}
...
func (chain *TestChain) CreateTMClientHeader(
	...
	trustedHeight clienttypes.Height,
	timestamp time.Time,
	cmtValSet, nextVals, tmTrustedVals *cmttypes.ValidatorSet,
	signers map[string]cmttypes.PrivValidator,
) *ibctm.Header {
	var (
...
func (chain *TestChain) GetChannelCapability(portID, channelID string) *capabilitytypes.Capability {
	capability, ok := chain.App.GetScopedIBCKeeper().
...
func (chain *TestChain) DeleteKey(key []byte) {
	storeKey := chain.App.GetKey(exported.StoreKey)
